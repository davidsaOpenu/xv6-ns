...
int protect_memory(struct cgroup* src, struct cgroup* dst, int proc_size) {
    ...
    // protected memory pages for dst cgroup and increas for src
    int proc_pg = PGROUNDUP(proc_size) / PGSIZE;
    int dec = calc_dst_protect_pg(dst, proc_pg);
    int inc = calc_src_protect_pg(src, proc_pg);
...
int calc_dst_protect_pg(struct cgroup* cgroup, int proc_pg)
{
...
int calc_src_protect_pg(struct cgroup* cgroup, int proc_pg)
{
