...
int cgroup_delete(char * path, char * type)
{
    ...
    *(cgp->cgroup_dir_path) = '\0';
    ...
    if (cgp != cgroup_root() && cgp -> mem_controller_enabled)
        decrese_protect_counter(cgp->protected_mem);
    return 0;
}
...
int protect_memory(struct cgroup* src, struct cgroup* dst, int proc_size) {
    ...
    int dec = decrese_cgroup_protect_memory(dst, proc_size);
    int inc = increse_cgroup_protect_memory(src, proc_size);
    if (increse_protect_counter (inc - dec) == 0) {
        dst->protected_mem = inc;
        src->protected_mem = dec <= 0 ? 0 : src->protected_mem - dec;
        return 0;
    }
    return 1;
}

int decrese_cgroup_protect_memory(struct cgroup* cgroup, int proc_size)
{
    if (cgroup != cgroup_root() && cgroup->mem_controller_enabled && cgroup->protected_mem > 0) {
        int pg = PGROUNDUP(proc_size) / PGSIZE;
        int used_pg = cgroup->current_page + pg;
...
int increse_cgroup_protect_memory(struct cgroup* cgroup, int proc_size)
{
   if (cgroup != cgroup_root() && cgroup->mem_controller_enabled) {
      ...
      int used_pg = cgroup->current_page - pg;
      int need_protect = PGROUNDUP(cgroup->min_mem) / PGSIZE - used_pg;
      return need_protect > 0 ? need_protect : 0;
   }
   return 0;
}
