...
{
    // cprintf("#####in unsafe_io_action_check\n");
    ...
    // cprintf("#####in unsafe_io_action_check: curr->bytes=%d, pmax->bytes=%d, curr->ios=%d, pmax->ios=%d\n", curr->bytes, pmax->bytes, curr->ios, pmax->ios);
    if (pmax->bytes != IO_ACCOUNT_NO_LIMIT && accumulated_bytes_needed > pmax->bytes)
    {
        // cprintf("#####in unsafe_io_action_check: reached max, accumulated_bytes_needed=%d\n", accumulated_bytes_needed);
        return min(accumulated_bytes_needed - pmax->bytes, size);
    }
    return size;
}

void cgroup_io_update_frame(struct cgroup *cgroup, ioAccount *paccount)
{
    unsigned int clock_now = steady_clock_now();
    unsigned int current_frame = clock_now / cgroup->io_account_period;
    // cprintf("#####cgroup_io_update_frame: frame #%d\n", current_frame);
...
int cgroup_request_io(struct cgroup *cgroup, short major, short minor, uint size, char is_write) {
    ...
    // cprintf("###cgroup_request_io: updating %s\n", cgroup->cgroup_dir_path);

    // Handle frame changes
    cgroup_io_update_frame(cgroup, paccount);
    ...
    while (allowed_io_size == 0) {
        // cprintf("###cgroup_request_io: Delaying......\n");
        // delay the io
    ...
    pcurr->ios++;
    pcurr->bytes += allowed_io_size;
    release(&cgroup->lock_cgroup_io_tables);
    // cprintf("###cgroup_request_io: allowed_io_size=%d requested_size=%d\n", allowed_io_size, size);
