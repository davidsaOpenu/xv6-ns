...
static void initialize_cgroup_depth(struct cgroup * cgroup)
{
    ...
    if (*parentdepth != 0)
        for (int i = 0;
             i < MAX_DEPTH_MAX_SIZE && (*depth++ = *parentdepth++) != 0;
             i++)
            ;
...
int disable_cpu_controller(struct cgroup * cgroup)
{
    ...
    for (int i = 1; i < sizeof(cgroups) / sizeof(cgroups[0]); i++)
        if (cgroups[i].parent == cgroup &&
            cgroups[i].cpu_controller_enabled)
            return -1;
...
void set_max_descendants_value(struct cgroup * cgroup, char * value)
{
    ...
    if (*value != 0)
        for (int i = 0; i < MAX_DECS_MAX_SIZE &&
...
void set_max_depth_value(struct cgroup * cgroup, char * value)
{
    ...
    if (*value != 0)
        for (int i = 0; i < MAX_DEPTH_MAX_SIZE &&
...
void set_nr_descendants(struct cgroup * cgroup, char * value)
{
    ...
    if (*value != 0)
        for (int i = 0;
             i < NR_DESC_MAX_SIZE && (*nr_descendants++ = *value++) != 0;
             i++)
            ;
}
...
void set_nr_dying_descendants(struct cgroup * cgroup, char * value)
{
    ...
    if (*value != 0)
        for (int i = 0; i < NR_DYING_DESC_MAX_SIZE &&
...
void get_cgroup_names_at_path(char * buf, char * path)
{
    ...
    for (int i = 1; i < sizeof(cgroups) / sizeof(cgroups[0]); i++)
        if (*(cgroups[i].cgroup_dir_path) != 0 &&
            strcmp(cgroups[i].parent->cgroup_dir_path, path) == 0) {
            char * child_name =
                &(cgroups[i].cgroup_dir_path[strlen(path) + 1]);
