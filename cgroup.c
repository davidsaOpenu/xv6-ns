...
int unsafe_io_action_check(const ioAction *curr, const ioAction *max, int size)
{
    if ((max->ios != IO_ACCOUNT_NO_LIMIT && curr->ios >= max->ios) ||
        (max->bytes != IO_ACCOUNT_NO_LIMIT && curr->bytes >= max->bytes))
    {
        // IO limit was reached
        return 0;
    }
    int accumulated_bytes_needed = curr->bytes + size;
    if (accumulated_bytes_needed > max->bytes)
    {
        return accumulated_bytes_needed - max->bytes;
    }
    return size;
}
