...
int unsafe_get_sum_children_weights(struct cgroup* cgroup) {
    int num_children_procces = cgroup->num_of_procs;
    int total_weight = 0;

    // for each populated child cgroup - the weight of an unpopulated cgroup is insignificant
    for (int i = 1; i < sizeof(cgtable.cgroups) / sizeof(cgtable.cgroups[0]); i++) {
        if (cgtable.cgroups[i].parent == cgroup && cgtable.cgroups[i].populated) {
            num_children_procces -= cgtable.cgroups[i].num_of_procs; // update num of children procces
            total_weight += cgtable.cgroups[i].cpu_weight; // update toatal weight
            // if (cgroup == cgroup_root()) {
            //     cprintf("%d", cgtable.cgroups[i].cpu_weight );
            // }
        }
    }
    // if (cgroup == cgroup_root()) {
    //     cprintf("-%d\n", num_children_procces);
    // }

    // each procces is treated as if it was hosted in a separate cgroup woth the defualt weight
    total_weight += num_children_procces * DEFAULT_CGROUP_CPU_WEIGHT;
