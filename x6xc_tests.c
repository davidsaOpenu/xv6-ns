...
/* Verify ioctlcnt syscall attach
  ioctlcnt(fd, TTYGETS, DEV_ATTACH) should return 0
  ioctlcnt(fd, TTYSETS, DEV_ATTACH) should return 0
  ioctlcnt(fd, TTYGETS, DEV_ATTACH) should return 1

  ioctlcnt(fd, TTYSETS, DEV_DETACH) should return 0
  ioctlcnt(fd, TTYGETS, DEV_DETACH) should return 1
*/
int ioctlcnt_attach_test() {
  int tty_fd;
  char * tty_name = "tty0";
  ...
  if(ioctlcnt(tty_fd, TTYGETS, DEV_ATTACH)  < 0){
    printf(stderr, "step 1. %s failed TTYGETS / DEV_ATTACH \n",tty_name);
    close(tty_fd);
    return -1;
  }

  if(ioctlcnt(tty_fd, TTYSETS, DEV_ATTACH) < 0){
    printf(stderr, "step 2. %s failed TTYSETS / DEV_ATTACH \n",tty_name);
    close(tty_fd);
    return -1;
  }

  int status = ioctlcnt(tty_fd, TTYGETS, DEV_ATTACH);
  ...
  if(status == 0){
    ...
    printf(stderr, "step 3. ioctlcnt failed");
    close(tty_fd);
    return -1;
  }
  else if(status == 1){
    ...
    if(ioctlcnt(tty_fd, TTYSETS, DEV_DETACH) == 0){
    ...
    status = ioctlcnt(tty_fd, TTYGETS, DEV_DETACH);
    ...
    if(status == 0){
      ...
      printf(stderr, "step 3. ioctlcnt GETS / DETACH failed");
      close(tty_fd);
      return -1;
    }
    else if(status == 1){
...
/* Verify ioctlcnt syscall connect
   ioctlcnt(fd, TTYGETS, DEV_CONNECT) should return 0
   ioctlcnt(fd, TTYSETS, DEV_CONNECT) should return 0
   ioctlcnt(fd, TTYGETS, DEV_CONNECT) should return 1

   ioctlcnt(fd, TTYSETS, DEV_DISCONNECT) should return to console, 0
   ioctlcnt(fd, TTYGETS, DEV_DISCONNECT) should return 1
  
*/
int ioctlcnt_connect_test() {
  int tty_fd;
  char * tty_name = "tty0";
  ...
  if( ioctlcnt(tty_fd, TTYGETS, DEV_CONNECT) < 0){
    printf(stderr, "step 1. %s failed TTYGETS / DEV_CONNECT \n",tty_name);
    close(tty_fd);
    return -1;
  }

  if(ioctlcnt(tty_fd, TTYSETS, DEV_CONNECT) < 0){
    printf(stderr, "step 2. %s failed TTYSETS / DEV_CONNECT \n",tty_name);
    close(tty_fd);
    return -1;
  }

  int status = ioctlcnt(tty_fd, TTYGETS, DEV_CONNECT);

  if(ioctlcnt(tty_fd, TTYSETS, DEV_DISCONNECT) == 0){
...
*/
int ioctlcnt_wrong_device_test() {
  int tty_fd;
...
int ioctlcnt_args_test() {
  ...
  if(ioctlcnt(tty_fd, TTYGETS, 0) != -1){
    close(tty_fd);
    return -1;
  }

  if(ioctlcnt(tty_fd, 0, 1) != -1){
    close(tty_fd);
    return -1;
  }

  if(ioctlcnt(tty_fd, 0, 0) != -1){
    close(tty_fd);
    return -1;
  }
  return 0;
...
int main() {
  ...
  //ioctlcnt TESTS
  if(run_test(ioctlcnt_args_test, "ioctlcnt args test,") < 0)
    return -1;
  if(run_test(ioctlcnt_wrong_device_test, "ioctlcnt wrongdev test") < 0)
    return -1;
  if(run_test(ioctlcnt_console_test, "ioctlcnt console test") < 0)
    return -1;

  //ioctlcnt SCENARIO TESTS
  if(run_test(ioctlcnt_attach_test, "ioctlcnt attach test") < 0)
    return -1;
  if(run_test(ioctlcnt_connect_test, "ioctlcnt connect test") < 0)
    return -1;
  
  printf(stderr, "ioctlcnt TESTS PASS:\n");
  exit(0);
}
