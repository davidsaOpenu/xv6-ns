...
/* Verify ioctl syscall attach/detatch
   verify TTYGETS&TTYSETS operations on attaching and detatching a tty device

   if given tty is not attached, TTYGETS & DEV_ATTACH/DEV_DETACH will return 0
   when we attach the given tty TTYGETS & DEV_ATTACH will return > 0
...
/* Verify ioctl syscall connect/disconnect
   verify TTYGETS&TTYSETS operations on connecting and disconnecting a tty device

   if given tty is not connected, TTYGETS & DEV_CONNECT/DEV_DISCONNECT will return 0
   when we connect the given tty TTYGETS & DEV_CONNECT will return > 0
   TTYSETS return 0 on operation success.
*/
