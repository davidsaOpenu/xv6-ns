...
int mutex_lock(struct mutex * mtx)
{
    ...
    /* We get here only after we have the lock, we want to make sure that no other
    memory operation happens (load/store) before the xchg instruction is done.
    Refer to spinlock.c for more exaplaination
    */
    __sync_synchronize();
    return 0;
}
