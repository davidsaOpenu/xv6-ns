...
#define INTERNAL_CMD_NONE -1
#define INTERNAL_CMD_DOLLAR_QUESTION 1
#define INTERNAL_CMD_CD 2
#define INTERNAL_CMD_EXIT 3
#define INTERNAL_CMD_CONNECT_TTY 4
#define INTERNAL_CMD_DISCONNECT_TTY 5
#define INTERNAL_CMD_ATTACH_TTY 6
#define INTERNAL_CMD_PID 7
...
int get_internal_cmd_type(struct execcmd *cmd)
{
  ...
  if ((strncmp(cmd->argv[0], "echo", 4) == 0) && (strncmp(cmd->argv[1], "$?", 2) == 0)) {
  ...
  } else if ((strncmp(cmd->argv[0], "connect", 7) == 0) && (strncmp(cmd->argv[1], "tty", 3) == 0)) {
    cmd_type = INTERNAL_CMD_CONNECT_TTY;
  } else if (strncmp(cmd->argv[0], "disconnect", 10) == 0) {
    cmd_type = INTERNAL_CMD_DISCONNECT_TTY;
  } else if ((strncmp(cmd->argv[0], "attach", 6) == 0) && (strncmp(cmd->argv[1], "tty", 3) == 0)) {
    cmd_type = INTERNAL_CMD_ATTACH_TTY;
  } else if (strncmp(cmd->argv[0], "pid", 3) == 0) {
