...
iget(uint dev, uint inum) {
    ...
    for (ip = &icache.inode[0]; ip < &icache.inode[NINODE]; ip++) {
        //cprintf("in loop in iget\n");
        if (ip->vfs_inode.ref > 0 && ip->vfs_inode.dev == dev && ip->vfs_inode.inum == inum) {
            ip->vfs_inode.ref++;
            //cprintf("in loop in iget before release\n");

            release(&icache.lock);
            //cprintf("in loop in iget after release\n");
    ...
    release(&icache.lock);
    //cprintf("in iget end after release\n");
