...
static int pouch_limit_cgroup(char* container_name, char* cgroup_state_obj, char * limitation){
    ...
    if(cpu_max_fd < 0){
        printf(2, "Incorrect cgroup object-state provided. Not applied.\n", container_name);
        return -1;
    }
...
static int print_clist(){
    ...
    for(i=0; i < NTTY; i++){
        ...
        if(read_from_pconf(tty,buf) <0 ){
            printf(2, "failed to read pconf from %s\n", tty);
            return -1;
        }
        if(!buf[0]){
            continue;
...
void printhelp_inside_cnt(){
    printf(2,"\nPouch commands inside containers:\n");
    printf(2,"       pouch disconnect \n");
    printf(2,"          : disconnect a currently connected container\n");
    printf(2,"       pouch info\n");
    printf(2,"          : query information about currently connected container\n");
}

void printhelp_outside_cnt(){
    printf(2,"\nPouch commands outside containers:\n");
    printf(2,"       pouch start {name}\n");
    printf(2,"          : starts a new container\n");
    printf(2,"          - {name} : container name\n");
    printf(2,"       pouch connect {name}\n");
    printf(2,"          : connect already started container\n");
    printf(2,"          - {name} : container name\n");
    printf(2,"       pouch destroy {name}\n");
    printf(2,"          : destroy a container\n");
    printf(2,"          - {name} : container name\n");
    printf(2,"       pouch info {name}\n");
    printf(2,"          : query information about a container\n");
    printf(2,"          - {name} : container name\n");
    printf(2,"       pouch list all\n");
    printf(2,"          : displays state of all created containers\n");
    printf(2,"      \ncontainers cgroups:\n");
    printf(2,"       pouch cgroup {cname} {state-object} [value]\n");
    printf(2,"          : limit given cgroup state-object\n");
    printf(2,"          - {name} : container name\n");
    printf(2,"          - {state-object} : cgroups state-object. Refer spec.\n");
    printf(2,"          - [value] : argument for the state-object, multiple values delimited by ','\n");
}

static int create_pouch_cgroup(char *cg_cname, char *cname){

    if(mkdir(cg_cname) != 0){
        printf(2,"Pouch: Failed to create cgroup with the given name. Consider another container name: %s \n",cname);
        return -1;
    }
    char cgpath[256];
...
main(int argc, char *argv[])
{
  ...
  if(argc >= 3){
     if((strcmp(argv[1],"--help") == 0) || (char)*argv[1] == '-'){
         if(ppid == 1)
             printhelp_inside_cnt();
         else
             printhelp_outside_cnt();
        exit(0);
     }
