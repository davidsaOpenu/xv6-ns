...
static int write_file_cg_subtree(struct file * f, char * addr, int n)
{
    ...
    if (cpucontroller == 1 && unsafe_enable_cpu_controller(f->cgp) == RESULT_FAILURE)
        return -1;
    if (cpucontroller == 2 &&
            unsafe_disable_cpu_controller(f->cgp) < 0)
        return -1;

    if (pidcontroller == 1 && unsafe_enable_pid_controller(f->cgp) == RESULT_FAILURE)
        return -1;
    if (pidcontroller == 2 &&
            unsafe_disable_pid_controller(f->cgp) == RESULT_FAILURE)
        return -1;

    if (setcontroller == 1 && unsafe_enable_set_controller(f->cgp) == RESULT_FAILURE)
        return -1;
    if (setcontroller == 2 &&
        unsafe_disable_set_controller(f->cgp) < 0)
        return -1;

    if (memcontroller == 1 && unsafe_enable_mem_controller(f->cgp) == RESULT_FAILURE)
        return -1;
    if (memcontroller == 2 &&
        unsafe_disable_mem_controller(f->cgp) == RESULT_FAILURE)
        return -1;
...
static int write_file_pid_max(struct file * f, char * addr, int n)
{
    char max_string[32] = {0};
        ...
        if (test != RESULT_SUCCESS_OPERATION)
            return -1;
        f->pid.max.max = max;
