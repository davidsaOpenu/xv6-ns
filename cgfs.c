...
static int get_file_name_constant(char * filename)
{
    if (strcmp(filename, "cgroup.procs") == 0)
        ...
        return SET_FRZ;
    else if (strcmp(filename, "memory.usage_in_bytes") == 0)
      return MEM_CUR;
    else if (strcmp(filename, "memory.limit_in_bytes") == 0)
      return MEM_MAX;
...
int unsafe_cg_read(cg_file_type type, struct file * f, char * addr, int n)
{
    if (type == CG_FILE){
        ...
        copy_and_move_buffer_max_len(&bufp, "memory.usage_in_bytes");
        ...
        if (f->cgp != cgroup_root()) {
            ...
            if (f->cgp->mem_controller_enabled) {
              copy_and_move_buffer_max_len(&bufp, "memory.limit_in_bytes");
            }
        }
