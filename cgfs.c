...
// Must be the biggest of the "MAX_" consts
#define MAX_STR 128
...
{
    // Used to hold the "resturn message" printed to the user
    char buf[MAX_STR];
    if (type == CG_FILE){
...
int unsafe_cg_write(struct file * f, char * addr, int n)
{
    ...
    if (f->writable == 0 || *f->cgp->cgroup_dir_path == 0 || n > MAX_STR)
        return -1;
