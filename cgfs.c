...
#define move_and_add(source, destination, var)               \
    memmove(source + var, destination, strlen(destination)); \
    var += strlen(destination);

#define change_controller_state(controller)                  \
    if (*addr == '+')                                        \
        controller = 1;                                      \
    if (*addr == '-')                                        \
        controller = 2;                                      \
...
#define copy_buffer_up_to(buffer, limit)                     \
    while (r < n && (r + f->off) < limit &&                  \
        (*addr++ = buffer[r++ + f->off]) != 0)               \
        ;

#define copy_buffer_up_to_newlines(buffer, limit)            \
    while (r < n && (r + f->off) < limit) {                  \
        if (!buffer[r + f->off]) {                           \
            *addr++ = '\n';                                  \
            ++r;                                             \
...
#define copy_and_move_buffer(buffer, string)                 \
    strncpy(buffer, string, sizeof(string));                 \
    buffer += strlen(string);

#define copy_and_move_buffer_max_len(buffer, string)         \
...
int unsafe_cg_read(cg_file_type type, struct file * f, char * addr, int n)
{
    if (type == CG_FILE){
        ...
        if (filename_const == CGROUP_PROCS) {
            ...
            copy_and_move_buffer(stattextp, "nr_dying_descandants - ")
            copy_and_move_buffer(stattextp, nr_dying_descendants_buf)
            copy_and_move_buffer(stattextp, "\n")

            copy_buffer_up_to(stattext, sizeof(stattext))
            ...
            copy_and_move_buffer(stattextp, buf)
            copy_and_move_buffer(stattextp, "\n")
            copy_and_move_buffer(stattextp, "user_usec - ")
            ...
            copy_buffer_up_to(stattext, strlen(stattext))
            ...
            copy_and_move_buffer(weighttextp, "weight - ")
            itoa(buf, f->cpu.weight.weight);
            copy_and_move_buffer(weighttextp, buf)
            copy_and_move_buffer(weighttextp, "\n")

            copy_buffer_up_to(weighttext, strlen(weighttext))

        } else if (filename_const == CPU_MAX) {
            char buf[11];
            char maxtext[strlen("max - ") + utoa(buf, f->cpu.max.max) +
                         strlen("\n") + strlen("period - ") +
                         itoa(buf, f->cpu.max.period) + 2];
            char * maxtextp = maxtext;

            copy_and_move_buffer(maxtextp, "max - ")
            utoa(buf, f->cpu.max.max);
            copy_and_move_buffer(maxtextp, buf)
            copy_and_move_buffer(maxtextp, "\n")
            copy_and_move_buffer(maxtextp, "period - ")
            itoa(buf, f->cpu.max.period);
            copy_and_move_buffer(maxtextp, buf)
            copy_and_move_buffer(maxtextp, "\n")

            copy_buffer_up_to(maxtext, strlen(maxtext))

        } else if (filename_const == PID_MAX) {
            char buf[11];
            char maxtext[strlen("max - ") + itoa(buf, f->pid.max.max) + 2];
            char * maxtextp = maxtext;

            copy_and_move_buffer(maxtextp, "max - ")
            itoa(buf, f->pid.max.max);
            copy_and_move_buffer(maxtextp, buf)
            copy_and_move_buffer(maxtextp, "\n")
...
int unsafe_cg_write(struct file * f, char * addr, int n)
{
    ...
    if (filename_const == CGROUP_PROCS) {
        ...
        while (*addr != '\0' && n > 0) {
            ...
            if (strcmp(buf, "cpu") == 0) {
                change_controller_state(cpucontroller)
            } else if (strcmp(buf, "pid") == 0) {
                change_controller_state(pidcontroller)
            } else
                return -1;
        }
