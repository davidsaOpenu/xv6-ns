...
static char cg_read_buf[MAX_STR];
static char cg_read_text_buf[MAX_STR];
...
/**
 * @brief Copies new line to provided buffer, the buffer index is advanced by one
 *
 * @param buffer Char parameter pointer to pointer
 */
static inline void copy_and_move_new_line_buffer(char** buffer)
{
    strncpy(*buffer, "\n", 1);
    *buffer += 1;
}
...
/**
 * @brief Copies null character to provided buffer, the buffer index is advanced by one
 *
 * @param buffer Char parameter pointer to pointer
 */
static inline void copy_and_move_null_char_buffer(char** buffer)
{
    strncpy(*buffer, "\0", 1);
    *buffer += 1;
}
...
/**
 * @brief Concatenates one string to provided another one
 *
 * @param dest String to concatenate to
 * @param source String to be concatenated
 * @return char* returns pointer to the new concatenated string
 */
...
{
    memset(cg_read_buf, '\0', MAX_STR);
    memset(cg_read_text_buf, '\0', MAX_STR);
    ...
    if (type == CG_FILE){
        ...
        if (filename_const == CGROUP_PROCS) {
            ...
            while (procoff < (sizeof(f->cgp->proc) / sizeof(*f->cgp->proc)) &&
                ...
                memset(cg_read_buf, '\0', MAX_PID_LENGTH);
                int pidlength = itoa(cg_read_buf, proc_pid(f->cgp->proc[procoff]));
                if (pidoff < pidlength) {
                    *addr = cg_read_buf[pidoff];
                    pidoff++;
                } else {
                    *addr = '\n';
                    pidoff = 0;
            ...
            if (f->cgp->cpu_controller_avalible) {
                move_and_add(cg_read_buf, "cpu\n", &i);
            }
            if (f->cgp->pid_controller_avalible) {
                move_and_add(cg_read_buf, "pid\n", &i);
            }
            if (f->cgp->set_controller_avalible) {
                move_and_add(cg_read_buf, "set\n", &i);
            }
            if (f->cgp->mem_controller_avalible) {
              move_and_add(cg_read_buf, "mem\n", &i);
            }

            r = copy_buffer_up_to_end(cg_read_buf + f->off, min(i, n), addr);
            ...
            if (f->cgp->populated) {
                strcat(cg_read_text_buf, "populated - 1\n");
            } else {
                strcat(cg_read_text_buf, "populated - 0\n");
            }

            if (f->cgp->is_frozen) {
                strcat(cg_read_text_buf, "frozen - 1\n");
            } else {
                strcat(cg_read_text_buf, "frozen - 0\n");
            }

            r = copy_buffer_up_to_end(cg_read_text_buf + f->off, min(sizeof(cg_read_text_buf), n), addr);

        } else if (filename_const == CGROUP_MAX_DESCENDANTS) {
            itoa(cg_read_buf, f->cgp->max_descendants_value);
            copy_buffer_up_to_end_replace_end_with_newline(cg_read_buf, min(sizeof(cg_read_buf), n), addr);

        } else if (filename_const == CGROUP_MAX_DEPTH) {
            itoa(cg_read_buf, f->cgp->max_depth_value);
            copy_buffer_up_to_end_replace_end_with_newline(cg_read_buf, min(sizeof(cg_read_buf), n), addr);

        } else if (filename_const == CGROUP_STAT) {
            uint read_buf_offset = 0;
            uint stat_val_len = 0;
            char *cg_read_buf_p = cg_read_buf;

            read_buf_offset += itoa(cg_read_buf_p + read_buf_offset, f->cgp->nr_descendants);
            read_buf_offset += itoa(cg_read_buf_p + read_buf_offset, f->cgp->nr_dying_descendants);

            char *stattextp = cg_read_text_buf;

            stat_val_len = uintlen(f->cgp->nr_descendants);
            copy_and_move_buffer(&stattextp, "nr_descendants - ", strlen("nr_descendants - "));
            copy_and_move_buffer(&stattextp, cg_read_buf, stat_val_len);
            copy_and_move_new_line_buffer(&stattextp);

            read_buf_offset = stat_val_len;
            stat_val_len = uintlen(f->cgp->nr_dying_descendants);
            copy_and_move_buffer(&stattextp, "nr_dying_descendants - ", strlen("nr_dying_descendants - "));
            copy_and_move_buffer(&stattextp, cg_read_buf + read_buf_offset, stat_val_len);

            copy_and_move_null_char_buffer(&stattextp);

            r = copy_buffer_up_to_end(cg_read_text_buf + f->off, min(stattextp - cg_read_text_buf, n), addr);
        } else if (filename_const == CPU_STAT) {
            uint read_buf_offset = 0;
            uint stat_val_len = 0;
            char *cg_read_buf_p = cg_read_buf;

            read_buf_offset += itoa(cg_read_buf_p + read_buf_offset, f->cpu.stat.usage_usec);
            read_buf_offset += itoa(cg_read_buf_p + read_buf_offset, f->cpu.stat.user_usec);
            read_buf_offset += itoa(cg_read_buf_p + read_buf_offset, f->cpu.stat.system_usec);
            read_buf_offset += itoa(cg_read_buf_p + read_buf_offset, f->cpu.stat.nr_periods);
            read_buf_offset += itoa(cg_read_buf_p + read_buf_offset, f->cpu.stat.nr_throttled);
            read_buf_offset += itoa(cg_read_buf_p + read_buf_offset, f->cpu.stat.throttled_usec);

            char *stattextp = cg_read_text_buf;

            stat_val_len = uintlen(f->cpu.stat.usage_usec);
            copy_and_move_buffer(&stattextp, "usage_usec - ", strlen("usage_usec - "));
            copy_and_move_buffer(&stattextp, cg_read_buf, stat_val_len);
            copy_and_move_new_line_buffer(&stattextp);

            read_buf_offset = stat_val_len;
            stat_val_len = uintlen(f->cpu.stat.user_usec);
            copy_and_move_buffer(&stattextp, "user_usec - ", strlen("user_usec - "));
            copy_and_move_buffer(&stattextp, cg_read_buf + read_buf_offset, stat_val_len);
            copy_and_move_new_line_buffer(&stattextp);

            read_buf_offset = stat_val_len;
            stat_val_len = uintlen(f->cpu.stat.system_usec);
            ...
            if (f->cpu.stat.active) {
                read_buf_offset = stat_val_len;
                stat_val_len = uintlen(f->cpu.stat.nr_periods);
                copy_and_move_buffer(&stattextp, "nr_periods - ", strlen("nr_periods - "));
                copy_and_move_buffer(&stattextp, cg_read_buf + read_buf_offset, stat_val_len);
                copy_and_move_new_line_buffer(&stattextp);

                read_buf_offset = stat_val_len;
                stat_val_len = uintlen(f->cpu.stat.nr_throttled);
                copy_and_move_buffer(&stattextp, "nr_throttled - ", strlen("nr_throttled - "));
                copy_and_move_buffer(&stattextp, cg_read_buf + read_buf_offset, stat_val_len);
                copy_and_move_new_line_buffer(&stattextp);

                read_buf_offset = stat_val_len;
                stat_val_len = uintlen(f->cpu.stat.throttled_usec);
            ...
            char *weighttextp = cg_read_text_buf;

            itoa(cg_read_buf, f->cpu.weight.weight);

            copy_and_move_buffer(&weighttextp, "weight - ", strlen("weight - "));
            copy_and_move_buffer(&weighttextp, cg_read_buf, strlen(cg_read_buf));
            copy_and_move_new_line_buffer(&weighttextp);
            copy_and_move_null_char_buffer(&weighttextp);
            ...
            read_buf_offset += itoa(cg_read_buf_p + read_buf_offset, f->cpu.max.max);
            read_buf_offset += itoa(cg_read_buf_p + read_buf_offset, f->cpu.max.period);

            char *maxtextp = cg_read_text_buf;

            stat_val_len = uintlen(f->cpu.max.max);
            copy_and_move_buffer(&maxtextp, "max - ", strlen("max - "));
            copy_and_move_buffer(&maxtextp, cg_read_buf, stat_val_len);
            copy_and_move_new_line_buffer(&maxtextp);

            read_buf_offset = stat_val_len;
            stat_val_len = uintlen(f->cpu.max.period);
            copy_and_move_buffer(&maxtextp, "period - ", strlen("period - "));
            copy_and_move_buffer(&maxtextp, cg_read_buf + read_buf_offset, stat_val_len);
            copy_and_move_new_line_buffer(&maxtextp);

            copy_and_move_null_char_buffer(&maxtextp);

            r = copy_buffer_up_to_end(cg_read_text_buf + f->off, min(maxtextp - cg_read_text_buf, n), addr);

        } else if (filename_const == PID_MAX) {
            char *maxtextp = cg_read_text_buf;

            itoa(cg_read_buf, f->pid.max.max);

            copy_and_move_buffer(&maxtextp, "max - ", strlen("max - "));
            copy_and_move_buffer(&maxtextp, cg_read_buf, strlen(cg_read_buf));
            copy_and_move_new_line_buffer(&maxtextp);
            ...
            itoa(cg_read_buf, f->cgp->num_of_procs);

            copy_and_move_buffer(&stattextp, "num_of_procs - ", strlen("num_of_procs - "));
            copy_and_move_buffer(&stattextp, cg_read_buf, strlen(cg_read_buf));
            copy_and_move_new_line_buffer(&stattextp);
            ...
            char * cputextp = cg_read_text_buf;

            itoa(cg_read_buf, f->cpu_s.set.cpu_id);

            copy_and_move_buffer(&cputextp, "use_cpu - ", strlen("use_cpu - "));
            copy_and_move_buffer(&cputextp, cg_read_buf, strlen(cg_read_buf));
            copy_and_move_new_line_buffer(&cputextp);

            copy_and_move_null_char_buffer(&cputextp);

            r = copy_buffer_up_to_end(cg_read_text_buf + f->off, min(cputextp - cg_read_text_buf, n), addr);
        } else if (filename_const == SET_FRZ) {
            char *frztextp = cg_read_text_buf;

            itoa(cg_read_buf, f->frz.freezer.frozen);

            copy_and_move_buffer(&frztextp, cg_read_buf, strlen(cg_read_buf));
            copy_and_move_new_line_buffer(&frztextp);

            copy_and_move_null_char_buffer(&frztextp);

            r = copy_buffer_up_to_end(cg_read_text_buf + f->off, min(frztextp - cg_read_text_buf, n), addr);
        } else if (filename_const == MEM_CUR) {
            utoa(cg_read_buf, f->cgp->current_mem);
            ...
            utoa(cg_read_buf, f->mem.max.max);
