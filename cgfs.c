...
int unsafe_cg_read(cg_file_type type, struct file * f, char * addr, int n)
{
    if (type == CG_FILE){
        ...
        // }
            ...
            copy_and_move_buffer(&stattextp, "dev\ttty\trios\trbytes\twios\twbytes\n", strlen("dev\ttty\trios\trbytes\twios\twbytes\n"));
            for (int dev_i = 0; dev_i < NDEV; dev_i++) {
                for (int tty_i = 0; tty_i < MAX_TTY; tty_i++) {
                    tty_stat = f->io.stat.io_stat_table[dev_i][tty_i];
                    if (tty_stat.rbytes != 0 || tty_stat.rios != 0 || tty_stat.wbytes != 0 || tty_stat.wios != 0) {
                        ...
                        num_str_length = itoa(tmp_num_buff, tty_stat.rios);
                        copy_and_move_buffer(&stattextp, tmp_num_buff, num_str_length);
                        copy_and_move_buffer(&stattextp, "\t", strlen("\t"));

                        num_str_length = itoa(tmp_num_buff, tty_stat.rbytes);
                        copy_and_move_buffer(&stattextp, tmp_num_buff, num_str_length);
                        copy_and_move_buffer(&stattextp, "\t", strlen("\t"));

                        num_str_length = itoa(tmp_num_buff, tty_stat.wios);
                        copy_and_move_buffer(&stattextp, tmp_num_buff, num_str_length);
                        copy_and_move_buffer(&stattextp, "\t", strlen("\t"));

                        num_str_length = itoa(tmp_num_buff, tty_stat.wbytes);
                        copy_and_move_buffer(&stattextp, tmp_num_buff, num_str_length);
                        copy_and_move_buffer(&stattextp, "\n", strlen("\n"));
                    }
                }
