...
int unsafe_cg_write(struct file * f, char * addr, int n)
{
    ...
    if (filename_const == CGROUP_PROCS) {
        ...
        if (weight <= 0)
            return -1;
        else if (weight < MIN_CGROUP_CPU_WEIGHT)
        {
            weight = MIN_CGROUP_CPU_WEIGHT;
        } else if (weight > MAX_CGROUP_CPU_WEIGHT)
        {
            weight = MAX_CGROUP_CPU_WEIGHT;
        }
        f->cgp->cpu_weight = weight;
        r = n;
    } else if (filename_const == PID_MAX &&
