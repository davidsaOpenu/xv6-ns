...
int cpu_account_schedule_process_decision_by_weights(struct cpu_account * cpu, struct proc * p) {
    ...
    struct cgroup *cgroup = p->cgroup;
    int debug = 1 && get_pid_for_ns(p, p->nsproxy->pid_ns) != 1 && get_pid_for_ns(p, p->nsproxy->pid_ns) != 2 && cpu->cpu_account_frame %10 == 0;
    ...
    cprintf("cpu_account_schedule_process_decision_by_weights returning %d\n", rc);
