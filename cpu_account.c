...
int cpu_account_schedule_process_decision_by_weights(struct cpu_account * cpu, struct proc * p) {
    ...
    if (debug) {
        ...
        cprintf("###p->cpu_period_time * 100 / cpu->curr_frame_toal_process_cpu_time=%d\n", p->cpu_percent, p->cpu_period_time * 100 / cpu->curr_frame_toal_process_cpu_time);
    ...
    while (cgroup != 0) { // loop until the very first genaration
        ...
        if (debug) {
            ...
            cprintf("expected_cpu_precent=%d, ", expected_cpu_precent);
                                         ...
                                         struct proc * p)
{
    ...
    p->cpu_percent = p->cpu_period_time / cpu->curr_frame_toal_process_cpu_time;
}
