...
static unsigned int curr_frame_toal_process_cpu_time = 1;
...
int cpu_account_schedule_process_decision_by_weights(struct cpu_account * cpu, struct proc * p) {
    ...
    if (debug) {
        ...
        cprintf("%d --curr_frame_toal_process_cpu_time ", curr_frame_toal_process_cpu_time);
        cprintf(" ");
        ...
        cprintf("%d --p->cpu_percent %d \n",p->cpu_percent, p->cpu_period_time * 100 / curr_frame_toal_process_cpu_time);
    ...
    if (expected_cpu_precent < p->cpu_period_time * 100 / curr_frame_toal_process_cpu_time) {
        if (debug) {
            cprintf("returning 0");
            cprintf("\n");
        }
                                        ...
                                        struct proc * p)
{
    ...
    if (cpu->now / cpu->cpu_account_period > cpu->cpu_account_frame) {
        cpu->curr_frame_toal_process_cpu_time = 1;
        curr_frame_toal_process_cpu_time = 1;
    ...
    curr_frame_toal_process_cpu_time += cpu->process_cpu_time;
    release(&toal_process_cpu_time_lock);
