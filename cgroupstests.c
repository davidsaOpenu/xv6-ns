...
int test_changing_cpu_weight(char *parent_weight, char *child_weight)
{
  if (!(write_file(TEST_1_CPU_WEIGHT, parent_weight)))
  {
    printf(1, "Failed to write %s to %s", parent_weight, TEST_1_CPU_WEIGHT);
    return -1;
  }

  // Update child weight
  if (!(write_file(CHILD_1_CPU_WEIGHT, child_weight)))
  {
    printf(1, "Failed to write %s to %s", child_weight, CHILD_1_CPU_WEIGHT);
    return -1;
  }
  ...
  printf(1, "time_before: %d\n", time_before);
  int test_cpu_duration = 10000000;
  int pid = fork();
  // Child
  if (pid == 0)
  {
  ...
  int parent_cpu_time = ioctl(0, IOCTL_GET_PROCESS_CPU_TIME, 0);
  while (parent_cpu_time - p_time_before < test_cpu_duration)
  {
    parent_cpu_time = ioctl(0, IOCTL_GET_PROCESS_CPU_TIME, 0);
  }
  uint64_t parent_duration = ioctl(0, IOCTL_GET_TIME, 0) - time_before;
  ...
  uint64_t child_duration = temp_read(0);
  int child_cpu_precentage;
  int parent_cpu_precentage;
  int32_t parent_duration_int = (int32_t) parent_duration;
  int32_t child_duration_int = (int32_t) child_duration;
  ...
  if (parent_duration > child_duration) {
    ...
    // so parent_duration*100/(child_duration*2) equals the cpu precentage of the child
    // the child's cpu precentage should be around 75
    uint64_t tmp = 100*parent_duration;
    child_cpu_precentage = parent_duration_int*100;
    if(tmp != child_cpu_precentage) {
      printf(1, "integeroverflow!! in test_changing_cpu_weight\n");
      return -1;
    }
    child_cpu_precentage = child_cpu_precentage/(child_duration_int*2);
    parent_cpu_precentage = 100 - child_cpu_precentage;
    ...
    // so 100*(2*parent_duration - child_duration)/(2*parent_duration) equals the cpu precentage of the child
    // the child's cpu precentage should be around 25
    uint64_t tmp = 100*(2*parent_duration - child_duration);
    child_cpu_precentage = 100*(2*parent_duration_int - child_duration_int);
    if(tmp != child_cpu_precentage) {
      printf(1, "integeroverflow!! in test_changing_cpu_weight\n");
      return -1;
    }
    printf(1, "child_cpu_precentage: %d, tmp: %d %d\n\n", child_cpu_precentage, tmp);
    printf(1, "parent_duration: %d %d, ", parent_duration);
    printf(1, "child_duration: %d %d\n\n", child_duration);
    child_cpu_precentage = child_cpu_precentage/(2*parent_duration_int);
    parent_cpu_precentage = 100 - child_cpu_precentage;
  }
  printf(1, "\nchild cpu precentage: %d\n", child_cpu_precentage);
  printf(1, "parent cpu precentage: %d\n\n", parent_cpu_precentage);
  //printf(1, "parent_duration: %d, child_duration: %d\n\n", parent_duration, child_duration);
...
TEST(test_cpu_weight)
{
  ...
  int child_cpu_precentage = test_changing_cpu_weight("2000", "300");

  // the child's cpu precentage should be around 75
  //ASSERT_TRUE(child_cpu_precentage > 60);

  child_cpu_precentage = test_changing_cpu_weight("2000", "33");

  // the child's cpu precentage should be around 25
  ASSERT_TRUE(child_cpu_precentage < 40);
