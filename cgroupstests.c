...
// How much cpu prevent of the totall cpu has the child used
int test_child_cpu_precent()
{
  int parent_cpu_time_before = ioctl(0, IOCTL_GET_PROCESS_CPU_TIME, 0);
  ...
  if (pid == 0)
  {
    ...
    int child_duration = ioctl(0, IOCTL_GET_TIME, 0) - time_before;
  ...
  while (parent_cpu_time - parent_cpu_time_before < test_cpu_duration)
  {
    parent_cpu_time = ioctl(0, IOCTL_GET_PROCESS_CPU_TIME, 0);
  }
  int parent_duration = ioctl(0, IOCTL_GET_TIME, 0) - time_before;
  ...
  printf(1, "child cpu precentage: %d\n", child_cpu_precentage);
...
TEST(test_cpu_weight)
{
  ...
  char * backup_parent_weight = read_file(TEST_1_CPU_WEIGHT, 0);
  ASSERT_TRUE(backup_parent_weight);

  // Use a bit weight to make the other processes neglegable
  char* parent_weight = "2000";
  char* child_weight = "300";

  ASSERT_TRUE(write_file(TEST_1_CPU_WEIGHT, parent_weight));

  ASSERT_TRUE(write_file(CHILD_1_CPU_WEIGHT, child_weight));
  ...
  ASSERT_FALSE(write_file(TEST_1_CPU_WEIGHT, backup_parent_weight));
