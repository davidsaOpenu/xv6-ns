...
TEST(test_cant_move_over_mem_limit)
{
  ...
  // Fail count should be zero at this point
  ASSERT_FALSE(strcmp(read_file(TEST_1_MEM_FAILCNT, 0), "0\n"));
  ...
  ASSERT_FALSE(strcmp(read_file(TEST_1_MEM_FAILCNT, 0), "1\n"));
...
TEST(test_cpu_stat)
{
    ...
    // Father
        ...
        // Verify that the scpu time has changed because of the child's runing
        ...
        // Verify that the scpu time has no changed since the child removed
