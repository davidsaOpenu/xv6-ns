...
int test_changing_cpu_weight(char *parent_weight, char *child_weight)
{
  ...
  if (pid == 0)
  {
    ...
    while (child_cpu_time - p_time_before < 1000000)
    {
  ...
  // parent_duration*100/(child_duration*2) equals the cpu precentage of the child
  // the child's cpu precentage should be around 75
  int child_cpu_precentage = parent_duration * 100 / (child_duration * 2);
