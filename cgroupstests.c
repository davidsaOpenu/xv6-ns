...
void mount_cgroup_fs(char *path)
{
	mount(0, path, "cgroup");
}

void make_cgroup_directory(char *path)
{
	mkdir(path);
}

void delete_cgroup_directory(char *path)
{
	unlink(path);
}

void unmount_cgroup_fs(char *path)
{
	umount(path);
}
 ...
 * Make new cgroup directories and create a hirarchy at following paths:
 * 1)	"/cgroup/test1"
 * 2)	"/cgroup/test2"
...
/**
 * Opens the cgroup file at the path cgroup with name file_name and returns the file descriptor.
 */
int open_cgroup_file(char *path, char* file_name)
{
	char file_path[128];
...
void test_opening_cgroup_files(char *path, int *cgroup_procs_fd, int *cgroup_controllers_fd, int *cgroup_subtree_control_fd, int *cgroup_events_fd, int *cgroup_max_descendants_fd, int *cgroup_max_depth_fd, int *cgroup_stat_fd)
{
	*cgroup_procs_fd = open_cgroup_file(path, "cgroup.procs");
	*cgroup_controllers_fd = open_cgroup_file(path, "cgroup.controllers");
	*cgroup_subtree_control_fd = open_cgroup_file(path, "cgroup.subtree_control");
	*cgroup_events_fd = open_cgroup_file(path, "cgroup.events");
	*cgroup_max_descendants_fd = open_cgroup_file(path, "cgroup.max.descendants");
 ...
 */
void test_reading_cgroup_files(char *path)
{
	char buf[256];
	int cgroup_procs_fd = open_cgroup_file(path, "cgroup.procs");
	int cgroup_controllers_fd = open_cgroup_file(path, "cgroup.controllers");
	int cgroup_subtree_control_fd = open_cgroup_file(path, "cgroup.subtree_control");
	int cgroup_events_fd = open_cgroup_file(path, "cgroup.events");
	int cgroup_max_descendants_fd = open_cgroup_file(path, "cgroup.max.descendants");
...
/**
 * Open, write, print, and close all files in cgroup located at path. move current process to cgroup path, anable the cpu controller and print contents. Then move the process back to root. 
 */
void test_writing_cgroup_files(char *path)
{
	char buf[256];
	int cgroup_procs_fd = open_cgroup_file(path, "cgroup.procs");
	int cgroup_controllers_fd = open_cgroup_file(path, "cgroup.controllers");
	...
	printf(1, "-----------------------------------------\nReturning process #%d to root cgroup:\n-----------------------------------------\n", curpid);
...
void enable_cpu_controller(char *path)
{
	char buf[256];
	int cgroup_controllers_fd = open_cgroup_file(path, "cgroup.controllers");
	int cgroup_subtree_control_fd = open_cgroup_file(path, "cgroup.subtree_control");
...
void disable_cpu_controller(char *path)
{
	char buf[256];
	int cgroup_subtree_control_fd = open_cgroup_file(path, "cgroup.subtree_control");
