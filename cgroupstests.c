...
TEST(test_correct_mem_account_of_growth) {
  // Save current process memory size.
  char proc_mem[10];
  // Buffer to read contents from memory file.
  char saved_mem[10];
...
TEST(test_limiting_mem)
{
  ...
  // Restore memory limit to 134217728
  ASSERT_TRUE(test_write(TEST_1_MEM_MAX, "134217728"));

  // Check changes
  ASSERT_FALSE(strcmp(test_read(TEST_1_MEM_MAX, 0), "134217728\n"));
...
int main(int argc, char * argv[])
{
    ...
    run_test(test_correct_mem_account_of_growth);
