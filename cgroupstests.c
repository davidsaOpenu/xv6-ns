...
TEST(test_io_stat)
{
  ...
  // prepare the string to peint/write
  ...
  if (diskBefore) {
    ASSERT_UINT_EQ(diskAfter->rbytes, diskBefore->rbytes + NUM_BYTES_TO_TEST);
    ASSERT_UINT_EQ(diskAfter->wbytes, diskBefore->wbytes + NUM_BYTES_TO_TEST);
    ASSERT_UINT_EQ(diskAfter->rios, diskBefore->rios + 1);
    ASSERT_UINT_EQ(diskAfter->wios, diskBefore->wios + 1);
  }
  else {
    ASSERT_UINT_EQ(diskAfter->rbytes, NUM_BYTES_TO_TEST);
    ASSERT_UINT_EQ(diskAfter->wbytes, NUM_BYTES_TO_TEST);
    ASSERT_UINT_EQ(diskAfter->rios, 1);
    ASSERT_UINT_EQ(diskAfter->wios, 1);
  }
  ...
  if (screenBefore) {
    ASSERT_UINT_EQ(screenAfter->rbytes, screenBefore->rbytes );
    ASSERT_UINT_EQ(screenAfter->wbytes, screenBefore->wbytes + NUM_BYTES_TO_TEST );
    ASSERT_UINT_EQ(screenAfter->rios, screenBefore->rios);
    ASSERT_UINT_EQ(screenAfter->wios, screenBefore->rios + NUM_BYTES_TO_TEST);
  } else {
    ASSERT_UINT_EQ(screenAfter->rbytes, 0);
    ASSERT_UINT_EQ(screenAfter->wbytes, NUM_BYTES_TO_TEST);
    ASSERT_UINT_EQ(screenAfter->rios, 0);
    ASSERT_UINT_EQ(screenAfter->wios, NUM_BYTES_TO_TEST);
  }
}
