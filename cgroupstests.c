...
{
    // Enable freezer controller.
    ASSERT_TRUE(test_enable_controller(FRZ_CNT));

    // Update frozen.
    ASSERT_TRUE(test_write(TEST_1_SET_FRZ, "1"));

    // Check changes.
    ASSERT_FALSE(strcmp(test_read(TEST_1_SET_FRZ, 0), "frozen - 1\n"));

    // Restore frozen.
    ASSERT_TRUE(test_write(TEST_1_SET_FRZ, "0"));

    // Check changes.
    ASSERT_FALSE(strcmp(test_read(TEST_1_SET_FRZ, 0), "frozen - 0\n"));

    // Disable freezer controller.
    ASSERT_TRUE(test_disable_controller(FRZ_CNT));
}

TEST(test_frozen_not_running)
{
    // Enable freeze controller.
    ASSERT_TRUE(test_enable_controller(FRZ_CNT));
