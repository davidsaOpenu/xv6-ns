...
struct cgroup
{
    ...
    unsigned int mem_stat_file_dirty_aggregated; /* Amount of cached filesystem data that was modified but not yet written back to disk */
...
/**
 * @brief Decrements the cgroup Memory Controller stat of file_dirty_aggregated
 *
 * @param cgroup pointer to a cgroup
 */
void cgroup_mem_stat_file_dirty_aggregated_decr(struct cgroup* cgroup);
