...
fork(void)
{
  ...
  // In case trying to fork new process and the cgroup reached its process limit, given pid controller is enabled, return failure
  if (curproc->cgroup->pid_controller_enabled && (curproc->cgroup->num_of_procs + 1) > curproc->cgroup->max_num_of_procs)
    return -1;
