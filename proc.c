...
growproc(int n)
{
  ...
  if (n > 0) {
    ...
    // Update protected memory for cgroup
    // if n>0 we update inside of allocuvm function
    update_protect_mam(curproc->cgroup, curproc->sz, sz);
  }
  curproc->sz = sz;
...
exit(int status)
{
  ...
  *curproc->cwdp = 0;
  ...
  update_protect_mam(curproc->cgroup, curproc->sz, 0);
